...
usage() {
  cat <<'USAGE'
Usage:
  ./scripts/create_feature_api_service.sh --name <snake_case_plural> [options]
...
Options:
  ...
  --print-router-snippet Print router/path snippet for app_router.dart and route_path.dart.
  --help, -h             Show this help.

Examples:
  ./scripts/create_feature_api_service.sh --name rewards
  ./scripts/create_feature_api_service.sh --name demo_feature --print-di-snippet --print-router-snippet
  ./scripts/create_feature_api_service.sh --name menu_items --entity MenuItem
USAGE
}
...
import '../../features/${feature_name}/feature_${feature_name}.dart';
...
feature_root="${app_root}/lib/features/${feature_name}"
test_root="${app_root}/test/features/${feature_name}"
...
cat > "${feature_root}/feature_${feature_name}.dart" <<EOF
/// ${feature_pascal} feature barrel exports.
export 'di/${feature_name}_module.dart';
export 'data/datasources/${feature_name}_local_data_source.dart';
export 'data/datasources/${feature_name}_remote_data_source.dart';
...
export 'domain/usecases/get_${feature_name}_use_case.dart';
export 'presentation/bloc/${feature_name}_list_bloc.dart';
export 'presentation/bloc/${feature_name}_list_event.dart';
export 'presentation/bloc/${feature_name}_list_state.dart';
export 'presentation/pages/${feature_name}_page.dart';
export 'presentation/widgets/${feature_name}_card.dart';
EOF
...
class ${feature_pascal}RemoteDataSourceImpl implements ${feature_pascal}RemoteDataSource {
  ${feature_pascal}RemoteDataSourceImpl(this._apiService);

  final ApiClient _apiService;

  @override
  Future<List<${feature_pascal}Model>> get${feature_pascal}() async {
    final payload = await _apiService.getList('/${feature_name}');
...
class ${feature_pascal}ListBloc
    extends Bloc<${feature_pascal}ListEvent, ${feature_pascal}ListState>
    with LoggerMixin {
  ...
  Future<void> _onRequested(
    ...
    emit(state.copyWith(status: ${feature_pascal}ListStatus.loading));
...
import 'package:__APP_NAME__/features/${feature_name}/feature_${feature_name}.dart';
...
echo "Created feature barrel: lib/features/${feature_name}/feature_${feature_name}.dart"
if [[ ${with_tests} -eq 1 ]]; then
...
echo "2. Add route/path and page in lib/app/router/app_router.dart and route_path.dart."
